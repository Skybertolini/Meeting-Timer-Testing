...
// === VT Addon 1.59: alternating colors always + group labels like "Avsnittene 4 og 5" or "Avsnittene 10–12" ===
(function(){
  (function ensureCSS(){
    if (document.querySelector('style[data-vt-addon-1_59]')) return;
    const style = document.createElement('style');
    style.setAttribute('data-vt-addon-1_59','');
    style.textContent = `
  ...
  const rangeLabel = (nums)=>{
    ...
    return `Avsnittene ${sorted[0]}–${sorted[sorted.length-1]}`;
  };
  ...
  // Always alternate colors, ignore groups
  function applyAlternatingColors(){
    ...
    slots.forEach((slot, idx) => {
      ...
      if (idx % 2 === 1) slot.classList.add('alt');
    });
  }
  ...
  function applyAll(){ applyAlternatingColors(); layoutPinsWithSpacing(); bindSelectionMessage(); }
