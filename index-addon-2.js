// index-addon.js — v1.64
// Adds: (1) correct group labels in message, (2) includes Ramme/Les-skriftsted for single paragraphs with a/b ordering
// Keeps: strict two-tone (groups as one block), tighter icon offsets
(function(){
  // --- CSS: icons closer to the timeline ---
  (function ensureCSS(){
  ...
  // Strict two-tone: group shares first tone; flip after whole group
  function applyGroupAwareTwoToneStrict(){
    ...
    // reset any stray classes / inline colors
  ...
  function layoutPinsAndBindMessages(){
    ...
    const ord = window.__VT_ORD || new Map(); // Map(para => {frame:1/2, read:1/2})
    ...
    // lay out pins (keep spacing, use existing class names)
    slots.forEach((slot, idx)=>{
      ...
      const handler=()=>{
        ...
        if (group.length > 1){
          // Case 1: show only group label
          msg.textContent = rangeLabel(group);
          return;
        }
        // Case 2: single paragraph -> include Ramme/Les-skriftsted with a/b ordering
        const o = ord.get(p) || {};
        ...
        if (hasFrame && hasRead){
          ...
          const r = o.read  ?? 2;
          if ((f ?? 99) <= (r ?? 99)){
            msg.textContent = `${rangeLabel([p])} + Ramme og Les-skriftsted`;
          } else {
            msg.textContent = `${rangeLabel([p])} + Les-skriftsted og Ramme`;
          }
        } else if (hasFrame){
  ...
  function applyAll(){ applyGroupAwareTwoToneStrict(); layoutPinsAndBindMessages(); }
