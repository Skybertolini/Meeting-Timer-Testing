...
// Build vertical pin stacks per paragraph: order = read -> image -> frame
function applyVerticalPinStacks(){
  const tl = $('#timeline'); if (!tl) return;
  const slots = $all('.para-slot', tl); if (!slots.length) return;

  // Read known sets if available
  ...
  slots.forEach((slot, idx)=>{
    ...
    // If DOM already has old pins, remember that too
    ...
    // Remove previous stacks and hide old pins so they don't compete on layout
    slot.querySelectorAll('.pin-stack').forEach(n=>n.remove());
    slot.querySelectorAll('.read-pin, .image-pin, .frame-pin, i.read-pin, i.image-pin, i.frame-pin').forEach(n=> n.remove());
