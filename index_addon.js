...
function __vtToSet(maybe){
  if (maybe instanceof Set) return maybe;
  const out = new Set();
  if (!maybe) return out;
  const addNum = (v)=>{ const n = Number(String(v).match(/^(\d+)/)?.[0]||NaN); if (!isNaN(n)) out.add(n); };
  if (Array.isArray(maybe)){
    for (const v of maybe){
      if (v && typeof v === 'object' && 'from' in v && 'to' in v){
        const a = Number(v.from), b = Number(v.to);
        if (!isNaN(a) && !isNaN(b)){
  ...
  if (typeof maybe === 'object'){
    for (const k of Object.keys(maybe)) addNum(k);
    return out;
  }
  addNum(maybe);
  return out;
