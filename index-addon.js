// index-addon.js — v1.66
// New: persistent full message during playback (groups OR single with Ramme/Les in a/b order)
(function(){
  ...
  // Two-tone: groups as a block, flip after whole group
  ...
  // Pins + click messages (same behavior as 1.64)
  function layoutPinsAndBindMessages(){
    const tl=document.getElementById('timeline'); if(!tl) return;
    const slots=Array.from(tl.querySelectorAll('.para-slot')); if(!slots.length) return;

    const ord = window.__VT_ORD || new Map(); // Map p -> {frame, read} where a=1, b=2
    ...
    // expose for normalizer
    window.__VT_BUILD_MSG_FOR = buildMsgFor;
  }

  // Persistent message normalizer: keep full info during playback
  function observeMessageAndNormalize(){
    const msg = document.getElementById('message'); if(!msg) return;
    const slots = Array.from(document.querySelectorAll('#timeline .para-slot'));
    if (!slots.length) return;
    const buildMsgFor = window.__VT_BUILD_MSG_FOR || (p => `Avsnitt ${p}`);

    // Map paragraph -> group or itself is handled inside buildMsgFor

    const normalize = () => {
      const text = (msg.textContent || '').trim();
      // Try to extract current paragraph number (base code tends to set "Avsnitt N" or similar)
      const m = text.match(/^Avsnitt\s+(\d+)(?:\b|$)/);
      if (!m) return; // already a custom string? we could still recalibrate, but be conservative
      const p = Number(m[1]);
      const desired = buildMsgFor(p);
      if (desired && text !== desired){
        msg.textContent = desired;
      }
    };
